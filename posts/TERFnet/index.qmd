...
abstract: |
    Analyse en réseau de la réacosphère et de son rôle dans les attaques contre le Planning Familial
bibliography: "./TERFnet.bib"
execute: 
  echo: false
  warning: false
---
...
```{r}
#| label: Chargement et manipulation des données
...
::: {.callout-note}
Cet article est un reupload de l'article que j'avais publié sur [Medium](https://medium.com/@maxence.ouafik/le-planning-familial-lextrême-droite-et-les-hommes-enceints-autopsie-d-une-polémique-32f2a6530036) le 21 septembre 2022
:::
...
![Affiche du planning familial ayant déclenché la polémique ©LaurierTheFox](./images/affiche_laurier.jpg){width="354" #fig-affiche}
...
[![Photo par Kier In Sight sur Unsplash](./images/unsplash_network.jpg){width="666"}](https://unsplash.com/photos/5VJLeQ-TrKs)
...
Pour créer ce réseau, j'ai récolté **`{r} total_tweets` tweets** publiés entre le **16/08/22** et le **26/08/2022**.Ces tweets ont été identifiés en utilisant les mots-clés \"planning familial\" ainsi que le hashtag \"#PlanningFamilial\". Ils ont été rédigés par **`{r} total_users` utilisateur·ice·s différent·e·s**. 

Pour gagner en lisibilité, j'ai exclu les comptes qui n'étaient liés à aucun autre compte du réseau, ce qui a laissé un total de **`{r} cleaned_users`** utilisateur·ice·s connecté·e·s par **38 213** retweets. Le réseau a ensuite été généré en utilisant l'algorithme *ForceAtlas2* @jacomy2014. 
Dans cette méthode de visualisation, les nœuds se repoussent mutuellement mais sont attirés par leurs liens, à savoir leurs retweets. En d'autres termes, les utilisateurs qui sont les plus proches idéologiquement sont également les plus proches sur le réseau.

::: {.callout-important appearance="simple"}
 J'insiste ici sur le fait que le positionnement des nœuds ne relève pas d'un choix arbitraire de ma part, mais d'une solution optimale calculée par un algorithme.
:::

 Par ailleurs, j'ai paramétré le réseau pour que la taille des nœuds représente leur nombre retweets, les plus gros nœuds correspondant aux comptes les plus retweetés. Enfin, la couleur de chaque nœud dépend de sa communauté. 

![Réseau de retweets autour de la polémique sur l'affiche du planning familial montrant un homme enceint](./images/gephi_plot.png){#fig-network}

Un rapide coup d'œil sur la [@fig-network] permet de constater l'extrême polarisation du réseau; deux camps se détachent très nettement l'un de l'autre. Les nœuds roses et oranges à la droite du réseau représentent les militant·e·s féministes et LGBTQIA+ ainsi que les personnalités et groupe de gauche tels que la CGT ou Philippe Poutou. La gauche du graphique, de son côté, représente tous les adversaires de l'affiche: les néoconservateurs, dont les TERFs, en orange, l'extrême-droite en rose, avec fdesouche ou Sardine Ruisseau, l'alt-right en turquoise, avec le compte de Reconquête, le parti d'Eric Zemmour, la droite réactionnaire en kaki, représentée par le Figaro, le Rassemblement National en vert, avec divers députés et porte-parole, et les conspirationnistes identitaires en bleu, dont André Bercoff. 
...
Si l'on zoom sur le graphique (voir [@fig-zoom]), cette porosité devient évidente, avec des points oranges, les néoconservateurs, qui gravitent à proximité des gros comptes de la fachosphère, et des comptes de la fachosphère qui gravitent à proximité des comptes néoconservateurs. 

![Zoom sur la zone de convergence montrant la porosité entre les néoconservateurs (orange), la droite réactionnaire (kaki), l'extrême droite (rose), le Rassemblement national (vert), et l'alt-right (turquoise). De nombreux points de la fachosphère gravitent autour des gros comptes néoconservateurs et inversement.](./images/graph_zoom.png){#fig-zoom}
...
![Animation résumant le développement du réseau au fil du temps ; 5 phases sont délimitées](./images/animation.gif){#fig-animation}

1. La première phase correspond au tweet initial de LaurierTheFox, le concepteur de l'affiche. 
2. Ce dernier est rapidement retweeté par l'alt-right qui s'empare du sujet dans la phase 2, via les comptes de Gabrielle Cluzel, rédactrice en chef de Boulevard Voltaire, Laurence Trochu, du parti Reconquête ainsi que le compte officiel de Reconquête. 
3. Peu après, Marguerite Stern amplifie la polémique en retweetant des comptes qui gravitent autour de l'alt-right, ce qui initie la phase 3. 
4. Dans la phase 4, les comptes de gauche se réveillent, avec principalement Philippe Poutou qui prend la défense du planning contre l'extrême-droite. Dans le même temps, le réseau d'extrême-droite se développe et se diversifie.
5. Enfin, la phase 5 correspond à l'entrée en scène du Figaro, et à la stabilisation du réseau. 
