...
```{r}
#| include: false
#| label: Script des boîtes à moustache
source("./boxplot.R")
```

Comme expliqué dans le @sec-resum, seules les guidelines suédoises et finlandaises ont été recommandées par les auteurs, notamment en raison de leur rigueur méthodologique > 50%. Un rapide coup d'œil à leur grille de cotation [@taylor2024clinical1, p. 5] montre en effet que ces deux études se démarquent *clairement* du lot. J'ai représenté les scores des différentes guidelines sous la forme de boîtes à moustache dans la @fig-boxplot. Des explications sur l'interprétation de ce type de graphique se trouvent dans la @tip-boxplot, si nécessaire. 

```{r}
#| label: fig-boxplot
#| fig-cap: "Boîtes à moustache représentant la répartition des différentes évaluations, par domaine. Le nom des guidelines ainsi que le score exact peut être lu en passant son curseur (ou en appuyant) sur un point."
agree_boxplot
```

:::{#tip-boxplot .callout-tip collapse="true"}
...
Cette imagée, tirée de [Wikipedia](https://fr.m.wikipedia.org/wiki/Fichier:Boxplot_vs_PDF.svg), permet d'illustrer cela : 

![](https://upload.wikimedia.org/wikipedia/commons/thumb/1/1a/Boxplot_vs_PDF.svg/939px-Boxplot_vs_PDF.svg.png)
:::

On constate rapidement que les guidelines suédoises et finlandaises obtiennent le plus haut score parmi toutes les guidelines pour leur champ d'investigation. Les guidelines suédoises obtiennent en outre le score maximal pour leur clarté, leur rigueur ainsi que pour la participation des personnes concernées. Les finlandaises sont, de leur côté, à ce point bien notées pour leur applicabilité qu'elles en deviennent des valeurs aberrantes (ou *outliers*, en anglais). 
Par ailleurs, elles se détachent nettement des autres, quand on les compare à la position de la médiane. Par exemple, avec une rigueur de 71%, les guidelines suédoises obtiennent un score plus de trois fois supérieur au score médian de 23% ! 
